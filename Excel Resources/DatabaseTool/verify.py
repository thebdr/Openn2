...
Writes Output/Report/coverage.csv (one row per signal) + coverage.txt (findings).
...
from safetydb import config, staging, outputs, hardware
...
def write_csv(records: list, out_dir: str) -> str:
    ...
    path = os.path.join(out_dir, "coverage.csv")
...
def write_txt(records: list, findings: list, counts: dict, out_dir: str) -> tuple[str, dict]:
    os.makedirs(out_dir, exist_ok=True)
    path = os.path.join(out_dir, "coverage.txt")
...
def generate(out_dir: str | None = None, params_path: str | None = None) -> dict:
    ...
    out_dir = out_dir or os.path.join(_abs(params.get("output_dir", "Output")), "Report")
...
def main():
    ...
    ap.add_argument("--out", default=None, help="output dir (default Output/Report)")
    args = ap.parse_args()
    s = generate(args.out)
