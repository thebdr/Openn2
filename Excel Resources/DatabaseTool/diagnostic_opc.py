...
def fl_value(db: list, row) -> str:
    ...
    (own profinet for node rows; else the node whose I/Q range contains the address); the
    literal `false` (never mute) when no node resolves."""
    node = row if row.get("profinet_name") else block_builders.node_of(db, row)
