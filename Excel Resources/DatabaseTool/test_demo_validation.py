...
def main():
    ...
    check("P1 forward FAIL address mismatch", e and e.level == "FAIL" and "mismatch" in e.message)
    e = at("CAUSE&EFFECT MATRIX!F7")
    check("P1 forward FAIL device not found", e and e.level == "FAIL" and "not found" in e.message)
    e = at("CAUSE&EFFECT MATRIX!F9")
    check("P1 forward FAIL empty device key", e and e.level == "FAIL" and "empty device key" in e.message)
    ...
    check("P2 reverse FAIL ce_mandatory=yes absent", e and e.level == "FAIL" and "not found in C&E" in e.message)
    e = at("NET SAFETY 50!O4")
    check("P2 reverse WARN ce_mandatory=warn absent", e and e.level == "WARN")
    e = at("NET SAFETY 50!O5")
    check("P2 reverse FAIL fld-only partial (address differs, both sides)",
          e and e.level == "FAIL" and "different address" in e.message and "I9.9" in e.message)
    e = at("NET SAFETY 50!O6")
    check("P2 reverse FAIL addr-only partial (device differs, both sides)",
          e and e.level == "FAIL" and "different device" in e.message and "S19999" in e.message)
    e = at("NET SAFETY 50!O7")
    check("P2 reverse FAIL untyped + safety word", e and e.level == "FAIL" and "safety concept" in e.message)
    ...
    check("3 phase banners + params.json header",
          sum(1 for e in log if e.level == "PHASE") == 3
          and any(e.location == "params.json" for e in log))
