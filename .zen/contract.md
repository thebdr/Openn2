...
- **Description:** The TEMPLATE BUILDER: the Files tab's template mode for a `chain_reactions/templates.yaml` - extends [[C-019]]'s authoring tools over [[C-024]]'s engine, and must NEVER approve what a fire rejects. (1) LINT: every problem the renderer / engine would report, found WITHOUT firing and in EVERY branch of every template, through the renderer's own checks and messages; in a chosen rule's context also what the strict render raises for that rule (a missing field, an unknown loop table, a loop-column typo) and the rule's own problems (a template of the wrong kind, a table absent at its hook, a hook the run-plan never fires, a target that will not render). A render that succeeds but likely betrays the author is a WARNING: an unknown name in a predicate, a `where` reading its own loop var, the empty hole `{}`, the braced literal `{{$x}}`. (2) PREVIEW: one fire of the rule for one source row of its hook's Database, through the fire path's OWN steps, nothing added or written - the same rows (uid-stamped), the same text + path, the same finding a real fire gives. (3) Each hook's Database is DECLARED by the system's run-plan (`System.reaction_hooks`) and equals what the fire sees. (4) The VIEW places everything exactly: two-layer highlighting (YAML for the structure; the target language under the template constructs), squiggles + a problems list (a click selects the culprit), a rule/row context bar, autocomplete from the rule's context (its columns, loop vars, `_params` keys, templates, tables, directives, functions). (5) The SHIPPED system configuration opens READ-ONLY; 'Create project copy' writes the file to the resolver's tier 1 (it then overrides the shipped one for that project), never over an existing copy, and needs an open project. User decisions (the spec review): edit inside the Files tab; literal braces doubled (in [[C-024]]); row templates fully editable; the shipped file read-only + a project copy.
- **Evidence:** .zen/evidence/C-025.md
- **Verification:** automated → Pipeline5App/tests/unit/test_template_builder.py, Pipeline5App/tests/unit/test_template_doc.py, Pipeline5App/tests/unit/test_gui_template_mode.py, Pipeline5App/tests/unit/test_siemens_main_handlers.py | the lint finds each of 25 renderer errors on the renderer's own line (+ an untaken branch's), the context lint matches the strict render, previews equal REAL fires (rows, text + path, finding), the declared hook Databases equal the fired ones (the smoke's pin), positions are exact on the shipped authoring manual, and the real FilesPanel on an off-screen Tk root shows the layers, a rule's preview, a squiggle + its problem row + the jump, an accepted completion, the shipped file read-only until copied; manual: the user's pilot acceptance over templates.yaml (two-layer highlight, real columns + template names offered, a preview on a real SSOT row).
- **Status:** pending verification
