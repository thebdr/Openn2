...
- **Description:** A context-aware template editor piloting on chain_reactions/templates.yaml - the upgrade over Tempemplator's static .scl highlighting (xlSyntaxHighLight.bas). Composes existing machinery: the fx dialog (gui/expr_builder.py - lint squiggles, autocomplete, live preview against a REAL SSOT row) + FileXY's langs.json language highlighting. Two-layer highlight (target language UNDER template constructs: @directives, {expr} holes, template refs); context-aware autocomplete (the rule's source_table columns + params + loop vars + template names); live expansion preview against selected SSOT row(s). Reachable from the Files tab; iterate after the pilot. Extends C-019 - merge at promotion. Depends on [[C-024]]. The lint must match the CONSUMERS (round-6 implications): the preview renders STRICT against the selected row, and `$row.column` inside a @for table loop is checked against that table's columns - the renderer's schema check - so the builder never approves a template the engine rejects. PHASE A LANDED 2026-09-25 (the spec review's decisions: edit INSIDE the Files tab; literal braces DOUBLED `{{ }}` - now in [[C-024]]; row templates fully editable too; the shipped builtin templates.yaml is read-only - 'Create project copy' to `<project>/config_project/systems/<sid>/chain_reactions/templates.yaml`). It should also flag a `where` predicate reading the LOOP var (`@for $s in signals where $s.x = ...` silently matches 0 rows - the predicate sees the row's columns, not the loop var; C-024 refute round 16). PHASE B LANDED 2026-09-25 (the Tk-free core - see Evidence): `tempemplator.lint` / `lint_line` (the renderer's own checks through shared helpers - one message each - walked over EVERY branch, nothing evaluated; a rule's context adds the strict missing-field, unknown-table and loop-column errors; predicate unknowns, a `where` reading its loop var, the empty hole `{}` and a braced literal `{{$x}}` are WARNINGS); `engine.lint` (entry kinds, row shapes + values, the rule's own problems) and `engine.preview` (one fire for one row through the fire path's own `_spawned` / `_file_output` - rows uid-stamped via `Table.stamped` - nothing added or written); `System.reaction_hooks` (each hook -> the Database it sees, declared by the run-plan and pinned against the real fire; Siemens after_300 = staging's `staged_database` + validation_issues); `workbench/template_doc.py` (the document model: exact positions, the two-layer spans read through `tempemplator.outline` / `brace_runs` / `hole_parts`, problem placement, completions). Phase C = the Files tab view.
...
- **Evidence:** .zen/evidence/P-012.md
