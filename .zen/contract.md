...
- **Description:** The TEMPLATE BUILDER: the Files tab's template mode for a `chain_reactions/templates.yaml`, over [[C-024]]'s engine - it must NEVER approve what a fire rejects. (1) LINT: every problem the renderer / engine would report, found WITHOUT firing and in EVERY branch of every template - the renderer's own checks and messages (the block structure by a sibling walker, pinned against the renderer: each of its errors found on its own line), format specs included (a spec no value can satisfy; a spec with no expression) and the @use nesting depth whatever the templates' order; in a chosen rule's context also what the strict render raises for that rule (a missing field, an unknown loop table, a loop-column typo) and the rule's own problems (a template of the wrong kind, a table absent at its hook, a hook the run-plan never fires, a target that will not render or carries a literal ':'). A render that succeeds but likely betrays the author is a WARNING: an unknown name in a predicate, a `where` reading its own loop var, an @if reading a loop row's missing column, the empty hole `{}`, the braced literal `{{$x}}`. (2) PREVIEW: one fire of the rule for one source row through the fire path's OWN guards and steps (the hook check, the per-rule backstop, the template check, the condition, the render), nothing added or written - the same rows (uid-stamped), the same text + path, the same finding a real fire gives; a row the condition rejects still renders, flagged (a fire skips it). (3) The Database a rule is checked and previewed against is the one its fire GETS: the system DECLARES each hook's (`System.reaction_hooks` - Siemens after_300 = staging itself, run in memory over the current source documents; never the Database/ folder, which later phases re-save), and the builder models what the run-plan does before the fire - a database-less hook with business settled in (its record attached) and the hook's earlier add_rows rules' spawns (the in-hook cascade, through the fire's own per-rule step) - pinned against the REAL run-plan's fire (a second run after a first saved spawns; a settled before_300). (4) The VIEW places everything exactly (a folded `>` block or a multi-line quoted scalar approximately): two-layer highlighting (YAML for the structure; the system's `template_language` under the template constructs - no language baked into the App), squiggles + a problems list (a click selects the culprit), a rule/row context bar, notes (this document is not the templates.yaml a fire uses; a staging that would halt; a system that declares no hooks; spawns into a staged table reach the saved Database, not generation), autocomplete from the rule's context (its columns in the templates it renders, loop vars, `_params` keys, templates, tables, directives, functions; a `where` offers only its loop table's columns); a slow Database build runs in a worker - the editor stays live. (5) The SHIPPED system configuration is a READ-ONLY reference in every Files-tab viewer; what runs is the project's own copy ([[C-004]] scaffolds one per file) - 'Open project copy' goes to it, 'Create project copy' makes one where a project has none (resolver tier 1; never over an existing copy; needs an open project). User decisions (the spec review): edit inside the Files tab; literal braces doubled (in [[C-024]]); row templates fully editable; the shipped file read-only + a project copy. Adds two fields to the [[C-021]] System contract (`reaction_hooks`, `template_language`). Extends [[C-019]] (the ƒx builder's lint + autocomplete + preview; one shared completion popup) - merge when C-019 re-verifies on PL5.
- **Evidence:** .zen/evidence/C-025.md
- **Verification:** automated → Pipeline5App/tests/unit/test_template_builder.py, Pipeline5App/tests/unit/test_template_doc.py, Pipeline5App/tests/unit/test_gui_template_mode.py, Pipeline5App/tests/unit/test_siemens_main_handlers.py | the lint finds each of 27 renderer errors on the renderer's own line (+ an untaken branch's, a bad format spec's, an order-independent @use depth), the context lint matches the strict render, previews equal REAL fires (rows, text + path, finding, the backstop, an unfired hook, a `self` field, the in-hook cascade), the Session models the run-plan (declared loader, settle, cascade, the notes) and the REAL run-plan smoke pins it (the builder's after_300 Database = what the fire gets, on a second run after spawns were saved, a before_300 settled in), positions are exact on the shipped authoring manual and every scalar style (`|2-`, escaped quotes, `''`, `|+`, typed keys), completions stay in scope, and the real FilesPanel on an off-screen Tk root shows the layers, the worker-built preview, a squiggle + its problem row + the jump, an accepted completion, the shipped config read-only in every viewer until copied; manual: the user's pilot acceptance over templates.yaml (two-layer highlight, real columns + template names offered, a preview on a real row).
- **Status:** pending verification
