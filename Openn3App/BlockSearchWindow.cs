...
namespace Openn
{
    ...
    public class BlockSearchWindow : Window
    {
        ...
        private readonly Action<string, string> exportBlock;
        ...
        /// <summary>Export formats: label shown to the user, extension passed to the backend.</summary>
        private static readonly Tuple<string, string>[] Formats =
        {
            Tuple.Create("XML (.xml) - any block/type", "xml"),
            Tuple.Create("Auto - native source", "auto"),
            Tuple.Create("SCL source (.scl)", "scl"),
            Tuple.Create("STL/AWL source (.awl)", "awl"),
            Tuple.Create("DB source (.db)", "db"),
            Tuple.Create("UDT source (.udt)", "udt"),
        };

        public BlockSearchWindow(Func<IList<PlcBlockInfo>> readBlocks, Action<string, string> exportBlock)
        {
            ...
            formatBox = new ComboBox
            {
                ...
                SelectedIndex = 0, //XML default - works for every object
                ToolTip = "Output format. \"Auto\" exports each object in its own native source language. Explicit source formats (scl/awl/db/udt) only apply to matching blocks; non-matching selections are skipped with a note in the log.",
            };
        ...
        private async Task ExportSelectedAsync()
        {
            ...
            string format = ((Tuple<string, string>)formatBox.SelectedItem).Item2;

            SetBusy(true, "Exporting " + selected.Count + " block(s) as " + format.ToUpperInvariant() + "...");
            try
            {
                await TiaWorker.Run(() =>
                {
                    foreach (PlcBlockInfo block in selected)
                        exportBlock(block.Name, format);
                });
