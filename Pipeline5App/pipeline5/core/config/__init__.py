...
# APP_ROOT = the Pipeline5App dir (dev: 4 levels above this file; frozen: the bundle dir).
if getattr(sys, "frozen", False):
    APP_ROOT = getattr(sys, "_MEIPASS", None) or os.path.dirname(sys.executable)
else:
    # pipeline5/core/config/__init__.py -> config -> core -> pipeline5 -> Pipeline5App
    # (config.py became the config/ package at the PL5 move - one level deeper than PL4)
...
def read_config_csv(path: str, json_columns=()) -> list:
    ...
    file -> []."""
    from pipeline5.core.ssot_table import decode_cell
