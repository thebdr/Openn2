...
A `.csv` opens in the shared DataGrid, its cells EDITABLE in place (an `.xlsx` read-only, with a sheet
picker); a
...
carries a path strip + **Open externally** / **Open folder** buttons (`gui/extedit`). A CSV saves in
its own dialect (the SSOT's generated tables are regenerated by the next run - the source documents
and the config are the durable place). The Tk-free scan/filter/read/write logic lives in
`gui/files_view.py` (tested); this is the view.
...
class FilesPanel(ttk.Frame):
    ...
    def _show_doc(self, path: str) -> None:
        ...
        (UI_REFRESH_PLAN D). Text = the editable text pane + syntax highlighting; Object = the scalar
        editor with `…` path pickers and an explicit Save (disabled on a SHIPPED file - it saves)."""
        self._clear_editor()
        self._header(path)
    ...
    def _text_body(self, path: str) -> None:
        ...
        if template:
            ...
            self._template = template_mode.TemplateMode(text, context_bar, lower, mode=self._mode, path=path)
        elif kind[0]:
