...
This module keeps the historical `pipeline5.workbench.datagrid` surface - the widget, the pure helpers,
the constants - so the hosts (files_panel, db_explorer, findings_panel) and the test suite are
untouched. The `filexy_shim` bootstrap (shared with the highlight/object_editor shims) puts the
...
from pipeline5.workbench import filexy_shim  # noqa: F401  (sys.path + theme binding - must run first)
