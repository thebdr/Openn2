"""The Table a block builder returns - the clean, single thing the whole phase is about.

A Table is just `columns` (ordered headers) + `rows` (list of dicts keyed by column). A builder
fills it from the Database however it likes; the engine serializes it into the kept `$/#/%/@`
CreationInfo CSV (engine.py). No rules, no sidecars - the builder owns every value.
...
- a cell whose value is a list is the horizontal ITERATOR: written across consecutive cells; such
  a column is emitted last. (Pad it yourself in Python if a template needs a fixed width.)
"""
