...
def build_00_commissioning(db: Database) -> Table:
    """One commissioning-bypass network per Profinet IO-device node - the nodes that carry a
    diagnosis member (a non-empty name_in_db, i.e. those in PROFINET_NODES_ALARM/WARNING); the
    PLC/CM heads and their partner cards are skipped. Both the `00_Commissioning` DB member and the
    network title are the node's "<profinet_name> <profinet_ip>"."""
...
def _area_descriptions(db: Database) -> dict:
    """{area: CONCEPT description} - the staged areas_description is positional with matrix_areas per
    row, so the i-th area maps to the i-th '|'-joined description piece (first non-empty wins)."""
    out = {}
    for r in db.rows:
        areas = [a.strip() for a in str(r.get("matrix_areas", "")).split("|") if a.strip()]
        descs = [d.strip() for d in str(r.get("areas_description", "")).split("|")]
...
def build_03_zone_cumulative(db: Database) -> Table:
    ...
    for area in db.areas():
        ...
        for label, types in ZONE_GROUPS:
            ...
            if not srcs:
                continue
            db_name = next((d.strip() for d in str(srcs[0].get("datablocks", "")).split("|") if d.strip()), "")
...
def build_05_output_feedback(db: Database) -> Table:
    ...
    def _row(r):
        return int(r.get("_source_row", 0) or 0)
    ...
    for idx in order:
        ...
        for kq in kqs:
            for a in str(kq.get("matrix_areas", "")).split("|"):
                a = a.strip()
        ...
        row = {
            ...
            "NetworkComment": re.sub(r"\s+", " ", f"{kq0.get('matrix_areas', '')} Contactor Output "
...
def build_06_feedback_error(db: Database) -> Table:
    ...
    address range), chunked to the FB's 8 IN/OUT slots. The iterator (each KQ's name_in_db) names a
    member in BOTH 03_FDBACK_RAW (the FDBACK ERROR n inputs) and 03_FDBACK (the ERROR n outputs); it
    is padded to 8 with PAD. Commissioning_bypass is the node's '<profinet_name> <profinet_ip>'."""
...
def build_07_speed_control(db: Database) -> Table:
    """One 01_Speed_Control_SLS FB instance per safety-encoder pair (N1/2 + N2/2 grouped by numeric
    index = the sorter number nn). PH1/PH2 = the sensors' tags; the 04_SPEED outputs are the sensors'
    healthy members + the 'Safety Encoder Failure [ <FLD> ]' member (added to 04_SPEED by the now-fixed
    datablock rule); SPEED_ZERO -> SPEED_STATE_SEND.SEND_DATA.SORTER_{nn}_STOPPED."""
