...
data_only). The sheet is APPEND-ONLY in its IDENTITY columns (existing rows preserved; new cabinets
appended) while the DERIVED columns (Count, unused_bits, non_unique_bits) are recomputed and rewritten
every run from the real (Diag_Cabinet, Diag_Bit) data. Type-1 rows first, then Type-2.
"""
...
def _norm_cab(v) -> str:
    """A Diag_Cabinet cell -> the 3-digit key matching DiagBlock.cabinet_text (numeric zero-padded; a
...
def _block_row(b, derived: dict, default: tuple) -> list:
    ...
    are range strings."""
    c, unused, nonuniq = derived.get(b.cabinet_text, default)
