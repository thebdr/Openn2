...
def main():
    ...
    try:
        ...
        xlsx = config.out_path(tmp, "io_tags_dir", "PLCTags.xlsx")
        ...
        body = open(config.out_path(tmp, "blocks_import_dir", "01_Pushbutton.db"), encoding="utf-8").read()
        ...
        check("diagnosis CSV written (address-less rows included)",
              n == 2 and os.path.exists(config.out_path(tmp, "diagnosis_dir", "List_IO.csv")))
