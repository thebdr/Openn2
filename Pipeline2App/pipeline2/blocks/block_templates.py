...
`Shared/Templates/Tia Portal Software Blocks/*.xml` contain placeholders of the form
`!!key$$` (e.g. `!!NetworkComment$$`, `!!Error_memberOf:03_FDBACK$$`). This tool:
  ...
  keys    - scans every template for its distinct !!key$$ placeholders and writes
            `config_project/block_templates.json` = { template : { key : <binding> } }.
  ...
  staged  - exports the staged database to ProjectDocumentation/InformationDatabase/IODatabase.csv (open it in the
            Pipeline2 Files tab and use the regex row filter to prototype queries).

Usage:
  python -m pipeline2.blocks.block_templates keys
...
TEMPLATES_JSON = config.BLOCK_TEMPLATES_JSON
...
def dump_staged(csv_path: str | None = None, params_path: str | None = None) -> str:
    ...
    from pipeline2.core import staging  # local import so `keys` doesn't need the I/O List
    ...
    csv_path = csv_path or config.out_path(config.output_root(params), "io_database")
        ...
        for r in rows:
            ...
            w.writerow([r.get(c, "") for c in cols]
                       + [r.get("matrix_areas", ""), r.get("IsSorterArea", ""),
                          r.get("name_in_db", ""), r.get("name_in_tagtable", ""),
                          r.get("tagtable", ""), r.get("datablocks", ""), r.get("diag_desc", ""),
                          r.get("diag_block_name", ""), r.get("diag_block_template", ""),
                          r.get("subnet_name", ""),
...
def main():
    ...
    sub.add_parser("keys", help="scan templates -> config_project/block_templates.json (merge)")
    sp = sub.add_parser("staged", help="export the staged database to CSV for inspection")
    sp.add_argument("--out", default=None, help="output CSV path (default: ProjectDocumentation/InformationDatabase/IODatabase.csv under the output tree)")
    args = ap.parse_args()
