...
def _output_dir(params: dict | None = None) -> str:
    from pipeline2.core import config  # lazy: avoid an import cycle
    params = params if params is not None else config.load_params()
    return config.output_root(params)


def shell_path(params: dict | None = None) -> str:
    """The shell workbook path (SoftwareBlocks/CreationInfo/SoftwareBlocks.xlsm in the output tree)."""
    from pipeline2.core import config
    return config.out_path(_output_dir(params), "blocks_creation_dir", "SoftwareBlocks.xlsm")
...
def _shell_rows(stem: str, db: list, mode: str = "keep") -> list:
    ...
    from pipeline2.blocks import softwareblocks  # lazy: avoid an import cycle
...
def main():
    import argparse
    import sys as _sys
    _root = os.path.dirname(os.path.dirname(_HERE))   # Pipeline2App (run-directly support)
    if _root not in _sys.path:
        _sys.path.insert(0, _root)
    from pipeline2.core import config, staging
    ap = argparse.ArgumentParser(description="Generate the per-template shell workbook (SoftwareBlocks/CreationInfo/SoftwareBlocks.xlsm under the output tree).")
