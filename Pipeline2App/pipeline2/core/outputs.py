...
I/O tags -> a single TIA "PLC Tags" workbook (PlcTags/PLCTags.xlsx); each tag's
...
def write_io_tags(tables: dict, out_dir: str) -> int:
    """Single TIA PLC-tags workbook PlcTags/PLCTags.xlsx: a 'PLC Tags' sheet
    ...
    tag_dir = config.out_path(out_dir, "io_tags_dir")
...
# The >List_IO / >List_Logic column layout is config-driven (config_project/diagnosis/diagnosis_columns.csv);
...
def build_diagnosis_list_io(io_rows: list) -> list:
    ...
    config_project/diagnosis/diagnosis_columns.csv headers (each cell interpolated from the CentralDatabase
...
def write_diagnosis_list_io(rows: list, out_dir: str, filename: str = "List_IO.csv") -> int:
    diag_dir = config.out_path(out_dir, "diagnosis_dir")
...
def write_dbs(dbs: dict, out_dir: str) -> int:
    if not dbs:
        return 0
    db_dir = config.out_path(out_dir, "blocks_import_dir")
