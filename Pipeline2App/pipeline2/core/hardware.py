...
Deduced rules (golden: Shared/HardwareConfigBuilderData/TestData/Passing/{Stations,Modules}.csv):
...
def extract(io_rows: list, dtd: dict):
    ...
    def finalize():
        ...
        # DTD col-5 (Parameters) is applied by Open2App itself - NOT written here.
        ...
        # params are applied by Open2App itself, so the row carries no params here.
...
def _format2(rows: list, keys: list) -> str:
    ...
    one csv-quoted row per dict (fields with ',' or '"' are quoted, as Open2App's
...
def write_stations(stations: list, out_dir: str) -> int:
    hw = config.out_path(out_dir, "hardware_dir")
