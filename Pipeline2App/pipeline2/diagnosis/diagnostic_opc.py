...
`SoftwareBlocks/ImportReady/Diagnostic_for_OPC.scl` - one FUNCTION, one REGION per cabinet, each a
CabState call + one BoolToUDInt call per packed DWord (ALARM1/2, WARNING1/2). It also
writes the `DiagList_IO.csv` / `DiagList_Logic.csv` inventories (config-driven columns).

Per channel: IN_xx = the row's PLC binding (outputs.plc_binding), ML_xx = outputs.ml_value,
FL_xx = "PROFINET_NODES_ALARM"."<profinet_name> <profinet_ip>". Only assigned channels are
...
from pipeline2.core import config, staging, outputs
from pipeline2.blocks import block_builders
from pipeline2.blocks import blockshells
from pipeline2.diagnosis import diagnosis_rules

TEMPLATE = os.path.join(config.BLOCK_TEMPLATES_DIR, "TEMPLATE--v1.0--06_Diagnostic for OPC.scl")
...
def fl_value(db: list, row) -> str:
    """FL_xx mute term: the row's node's alarm bit in PROFINET_NODES_ALARM -
    "PROFINET_NODES_ALARM"."<profinet_name> <profinet_ip>" (the PA `db_element` shape, so it
    matches the member that actually lives in that DB). The node is the one whose I/Q range
    contains the signal's address; `false` (never mute) when no node resolves. A node-level
    alarm itself (PA/PW - it carries its own `profinet_name`) must NOT filter on itself ->
    `false`."""
...
def generate_for(db: list, params: dict, out_dir: str) -> dict:
    """Write DiagList_IO.csv + DiagList_Logic.csv + Diagnostic_for_OPC.scl from an already-staged
    CentralDatabase (so a pipeline run doesn't re-read the I/O List)."""
    blocks = staging.load_diagnostic_blocks(params)
    scl_dir = config.out_path(out_dir, "blocks_import_dir")
    os.makedirs(scl_dir, exist_ok=True)

    # DiagList_IO (auto) + DiagList_Logic (rule-generated) inventories
    io_rows = outputs.build_diagnosis_list_io(db)
    outputs.write_diagnosis_list_io(io_rows, out_dir, "DiagList_IO.csv")
    logic_rows, logic_entries = diagnosis_rules.build_list_logic(db, blocks)
    outputs.write_diagnosis_list_io(logic_rows, out_dir, "DiagList_Logic.csv")

    entries = io_entries(db) + logic_entries
    scl_path = os.path.join(scl_dir, "Diagnostic_for_OPC.scl")
...
def main():
    ap = argparse.ArgumentParser(description="Generate the Diagnostic-for-OPC SCL + DiagList_IO/DiagList_Logic.")
    ap.add_argument("--out", default=None)
    args = ap.parse_args()
    r = generate(args.out)
    print(f"diagnosis: {r['io']} DiagList_IO + {r['logic']} DiagList_Logic  ->  {r['scl']}")
