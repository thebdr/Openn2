...
`config_project/diagnosis/diagnosis_logic_rules.csv` rules each fire when ALL their `required_types`
...
from pipeline2.core import config, outputs
...
def load_rules() -> list:
    ...
    path = os.path.join(config.DIAGNOSIS_DIR, "diagnosis_logic_rules.csv")
    if not os.path.exists(path):
        return []
    rules = []
    for r in config._read_csv("diagnosis_logic_rules.csv", config.DIAGNOSIS_DIR):
...
def build_list_logic(db: list, blocks: dict) -> tuple[list, list]:
    ...
    from pipeline2.diagnosis.diagnostic_opc import fl_value, _is_warning  # lazy: avoid an import cycle
    rules = load_rules()
